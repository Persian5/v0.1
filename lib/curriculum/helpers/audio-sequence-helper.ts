 ...
 * @returns AudioSequenceStep with resolved sequence (surface IDs)
 * 
 * Examples:
...
export function audioSequenceHelper(
  ...
  // Resolve all lexemes in sequence to get surface IDs
  const resolvedSequence = sequence.map(ref => {
    const resolved = GrammarService.resolve(ref);
    return resolved.id; // Use surface ID (e.g., "khoobam" for grammar form)
  });
